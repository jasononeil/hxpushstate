...
var demo = demo || {}
demo.Test = $hxClasses["demo.Test"] = function() { }
demo.Test.__name__ = ["demo","Test"];
demo.Test.main = function() {
	pushstate.PushState.init();
	pushstate.PushState.onStateChange.bind(function(event) {
		demo.Test.stateChangeCount++;
		if(demo.Test.stateChangeCount > 1) new js.JQuery("#load-type").text("This content was a push-state");
		new js.JQuery("#content").text("I want to become a " + event.url);
	});
	new js.JQuery(js.Lib.document).ready(function(e) {
		new js.JQuery("#animal-form").submit(function(e1) {
			var value = $(this).find("input").val();
			pushstate.PushState.push("/" + value);
			e1.preventDefault();
		});
	});
}
